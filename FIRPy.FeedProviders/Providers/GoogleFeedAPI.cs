...
namespace FIRPy.FeedAPI
{
    public class GoogleFeedAPI : FeedProvider
    {
        ...
        public override TickReportData GenerateTickReportData(Ticks tick)
        {
            ...
            var prevDayData = tick.TickGroup.Where(x => !x.Date.ToShortDateString().Equals(DateTime.Today.ToShortDateString()));
            retData.Symbol = tick.Symbol;

            if (currentDayData.Count() > 0 && prevDayData.Count() > 0)
            {
                var currentVolume = currentDayData.Sum(v => v.Volume);
                var symbol = tick.Symbol;
                var prevClose = prevDayData.Last().Close;
                ...
                if (currentDayData.Count() >= 2)
                {
                    ...
                    var changeInVolume = Math.Round((Convert.ToDouble(changeInVolumeArray[0].Volume) - Convert.ToDouble(changeInVolumeArray[1].Volume)) / Convert.ToDouble(changeInVolumeArray[1].Volume) * 100, 0);
